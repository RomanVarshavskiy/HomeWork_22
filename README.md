...
- users — аутентификация и профили (регистрация, вход, редактирование профиля)
...
- В каталоге реализован счётчик просмотров на детальной странице товара.
- Права и роли:
    - владелец может редактировать и удалять свои товары/категории;
    - модератор может ограниченно редактировать (например, управление публикацией товара);
    - суперпользователь имеет полный доступ.
- Для корректной работы email‑уведомлений настройте параметры в `.env` (либо используйте консольный backend в настройках
  проекта).
...
. ├── catalog/ # Приложение каталога (модели, вьюхи, формы, urls, шаблоны, management) ├── myblog/ # Приложение блога (
модели, вьюхи, urls, шаблоны) ├── users/ # Приложение пользователей (auth, формы, шаблоны) │ └── templates/users/ #
login.html, register.html, profile_edit.html ├── config/ # Настройки проекта (settings, urls, wsgi, asgi) ├── media/ #
Загружаемые файлы (dev) ├── static/ # Статические файлы (если используются) ├── catalog_fixture.json # Фикстура с
тестовыми данными каталога ├── myblog_fixture.json # Фикстура с тестовыми данными блога ├── groups.json # Фикстура с
группами/правами (модераторы и т.п.) ├── .env_template # Шаблон переменных окружения ├── manage.py # Точка входа Django
├── pyproject.toml # Зависимости и настройки Poetry/линтеров └── README.md
